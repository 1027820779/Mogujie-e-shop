...
<html>
	...
	<body>
		...
		<div class="alert alert-success" style="text-align:center;"><a style="float:left;" href="{{U('index')}}" class="btn btn-sm btn-primary">&lt;-返回</a>编辑{{$oldData['bname']}}</div>
		<div class="form-group">
			<label for="exampleInputEmail1">名称</label>
			<input id="exampleInputEmail1" class="form-control" type="text"  name="bname" value="{{$oldData['bname']}}">
		</div>
		...
		<div id="">
			...
			<label class="checkbox checkbox-inline" for="checkbox">
				<input id="checkbox" class="custom-checkbox" type="radio" {{if value="$oldData['is_hot']==1"}}checked="checked"{{endif}} data-toggle="radio" value="1" name="is_hot"> 是
			</label>
			<label class="checkbox checkbox-inline" for="checkbox1">
				<input id="checkbox1" class="custom-checkbox" type="radio" data-toggle="radio" {{if value="$oldData['is_hot']==0"}}checked="checked"{{endif}} value="0" name="is_hot"> 否
			</label>
		</div>
		...
		<input type="hidden" name="bid" value="{{$oldData['bid']}}"/>

	...
	<body>
		...
		<table class="table table-hover">
				...
				<td>
					<select name="category_cid" class="form-control" rule="required" msg="所属分类必须要选择">
						...
						{{foreach from="$cateData" value="$v"}}
							<option value="{{$v['cid']}}" {{if value="$goods_all['category_cid']==$v['cid']"}} selected="selected" {{endif}} tid="{{$v['type_tid']}}">{{$v['_name']}}</option>
						{{endforeach}}
					</select>
				</td>			  
			</tr>
				...
				<td>
					<select name="brand_bid" class="form-control" rule="required" msg="所属品牌必须要选择">
						...
						{{foreach from="$brandData" value="$v"}}
							<option value="{{$v['bid']}}" {{if value="$goods_all['brand_bid']==$v['bid']"}} selected="selected" {{endif}}>{{$v['bname']}}</option>
						{{endforeach}}
					</select>
				</td>
			</tr>
			<tr>
			  <td>商品名称</td>
			  <td>
					<input type="text" name="gname" id="" value="{{$goods_all['gname']}}" class="form-control" rule="required" msg="商品名称必须要填"/>			  	
			  </td>
			</tr>
			<tr>
			  <td>货号</td>
			  <td>
					<input type="text" name="gnumber" id="" value="{{$goods_all['gnumber']}}" class="form-control" rule="required" msg="货号必须要选"/>			  	
			  </td>
			</tr>			
			<tr>
			  <td>单位</td>
			  <td>
					<input type="text" name="unit" id="" value="{{$goods_all['unit']}}" class="form-control"/>			  	
			  </td>
			</tr>
			<tr>
			  <td>市场价</td>
			  <td>
					<input type="text" name="mark_price" id="" value="{{$goods_all['mark_price']}}" class="form-control" rule="required" msg="市场价必须要填" />			  	
			  </td>
			</tr>
			<tr>
			  <td>商城价</td>
			  <td>
					<input type="text" name="goods_price" id="" value="{{$goods_all['goods_price']}}" class="form-control" rule="required" msg="商城价必须要填"/>			  	
			  </td>
			</tr>
			<tr>
			  <td>总库存</td>
			  <td>
					<input type="text" name="stock_num" id="" value="{{$goods_all['stock_num']}}" class="form-control" rule="required" msg="库存数必须要填"/>			  	
			  </td>
			</tr>			
			<tr>
			  <td>点击次数</td>
			  <td>
					<input type="text" name="click" id="" value="{{$goods_all['click']}}" class="form-control"/>			  	
			  </td>
			</tr>	
		</table>
		<table class="table table-hover">
		...
		<table class="table table-hover" id="attr">
			...
			<tr>
			  <td width="200">{{$v['atname']}}</td>
				<td>
					<select name="attr[{{$v["gtid"]}}][]" class="form-control">
					<option value="">-请选择-</option>
					{{foreach from="$v['attrvalue']" value="$vv"}}
						<option value="{{$vv}}" {{if value="$v['g_type_value']==$vv"}} selected="selected" {{endif}}>{{$vv}}</option>
					{{endforeach}}
					</select>
					<input type="hidden" name="attr[{{$v["gtid"]}}][attr_type_atid]" value="{{$v['attr_type_atid']}}">
				</td>	
			</tr>
			{{endif}}
			{{endforeach}}
...
<!-- 			<tr>
		...
		<table class="table table-hover" id="spec">
			{{foreach from="$data" value="$v"}}	
			{{if value="$v['attr_cate']==1"}}																							
			<tr>
				<td  width="200">{{$v['atname']}}</td>
				<td>
					<select name="spec[{{$v["gtid"]}}][]" class="form-control">
					<option value="">-请选择-</option>
					{{foreach from="$v['attrvalue']" value="$vv"}}
						<option value="{{$vv}}" {{if value="$v['g_type_value']==$vv"}} selected="selected" {{endif}}>{{$vv}}</option>
					{{endforeach}}	
				...
				<input type="hidden" name="spec[{{$v["gtid"]}}][attr_type_atid]" value="{{$v['attr_type_atid']}}">
				<td width="280">
					<input type="text" name="spec[{{$v["gtid"]}}][g_other_price]" id="" value="{{$v['g_other_price']}}" placeholder='附加价格' class="form-control"/>	
				</td>
				<td style="text-align: center;">
		...
		<div lab="uploadFilee" class="uploadFilee">
		    ...
		    <div id="uploadList">
		        <ul>
		        	{{if value="$goods_all['list_pic']!=''"}}
					<li>
					<img src='{{$goods_all["list_pic"]}}'/>
					<input type='hidden' name='list_pic' value='{{$goods_all["list_pic"]}}'/>
					</li>
					{{endif}}
		        </ul>
		    </div>
		...
		<!-- html页面 -->
		<div lab="uploadFilee" class="uploadFilee">
        	{{if value="empty($con_data['m_pics'])"}}
		    ...
		    <div id="uploadLists">
		        <ul>
	        	{{if value="!empty($con_data['m_pics'])"}}
		        {{foreach from="$con_data['m_pics']" value="$v"}}
		        	<li>
			        	<img src="{{$v}}"/>
		...
		<table class="table table-hover">
			...
			<tr>
				<!-- 调用百度编辑器 -->
				<script id="editor" type="text/plain" style="width:100%;height:auto;" name="goods_content" rule="required" msg="商品详细必须要填">{{$con_data['goods_content']}}</script>
				<script>
		...
		<table class="table table-hover">
			...
			<tr>
				<!-- 调用百度编辑器 -->
				<script id="editor2" type="text/plain" style="width:100%;height:auto;" name="goods_service">{{$con_data['goods_service']}}</script>
				<script>
		...
		<input type="hidden" name="type_tid" value="{{$tid}}"/>

	...
	<body>
		...
		<table class="table table-hover">
				...
				<td>
					<select name="category_cid" class="form-control" rule="required" msg="所属分类必须要选择">
						...
						<?php foreach ($cateData as $v){?>
							<option value="<?php echo $v['cid']?>" <?php if($goods_all['category_cid']==$v['cid']){?>
                 selected="selected" 
               <?php }?> tid="<?php echo $v['type_tid']?>"><?php echo $v['_name']?></option>
						<?php }?>
					</select>
				</td>			  
			</tr>
				...
				<td>
					<select name="brand_bid" class="form-control" rule="required" msg="所属品牌必须要选择">
						...
						<?php foreach ($brandData as $v){?>
							<option value="<?php echo $v['bid']?>" <?php if($goods_all['brand_bid']==$v['bid']){?>
                 selected="selected" 
			...
			<tr>
			  <td>商品名称</td>
			  <td>
					<input type="text" name="gname" id="" value="<?php echo $goods_all['gname']?>" class="form-control" rule="required" msg="商品名称必须要填"/>			  	
			  </td>
			</tr>
			<tr>
			  <td>货号</td>
			  <td>
					<input type="text" name="gnumber" id="" value="<?php echo $goods_all['gnumber']?>" class="form-control" rule="required" msg="货号必须要选"/>			  	
			  </td>
			</tr>			
			<tr>
			  <td>单位</td>
			  <td>
					<input type="text" name="unit" id="" value="<?php echo $goods_all['unit']?>" class="form-control"/>			  	
			  </td>
			</tr>
			<tr>
			  <td>市场价</td>
			  <td>
					<input type="text" name="mark_price" id="" value="<?php echo $goods_all['mark_price']?>" class="form-control" rule="required" msg="市场价必须要填" />			  	
			  </td>
			</tr>
			<tr>
			  <td>商城价</td>
			  <td>
					<input type="text" name="goods_price" id="" value="<?php echo $goods_all['goods_price']?>" class="form-control" rule="required" msg="商城价必须要填"/>			  	
			  </td>
			</tr>
			<tr>
			  <td>总库存</td>
			  <td>
					<input type="text" name="stock_num" id="" value="<?php echo $goods_all['stock_num']?>" class="form-control" rule="required" msg="库存数必须要填"/>			  	
			  </td>
			</tr>			
			<tr>
			  <td>点击次数</td>
			  <td>
					<input type="text" name="click" id="" value="<?php echo $goods_all['click']?>" class="form-control"/>			  	
			  </td>
			</tr>	
		</table>
		<table class="table table-hover">
		...
		<table class="table table-hover" id="attr">
			...
			<tr>
			  <td width="200"><?php echo $v['atname']?></td>
				<td>
					<select name="attr[<?php echo $v["gtid"]?>][]" class="form-control">
					<option value="">-请选择-</option>
					<?php foreach ($v['attrvalue'] as $vv){?>
						<option value="<?php echo $vv?>" <?php if($v['g_type_value']==$vv){?>
                 selected="selected" 
               <?php }?>><?php echo $vv?></option>
					<?php }?>
					</select>
					<input type="hidden" name="attr[<?php echo $v["gtid"]?>][attr_type_atid]" value="<?php echo $v['attr_type_atid']?>">
				</td>	
			</tr>
...
<!-- 			<tr>
		...
		<table class="table table-hover" id="spec">
			<?php foreach ($data as $v){?>	
			<?php if($v['attr_cate']==1){?>
                																							
			<tr>
				<td  width="200"><?php echo $v['atname']?></td>
				<td>
					<select name="spec[<?php echo $v["gtid"]?>][]" class="form-control">
					<option value="">-请选择-</option>
					<?php foreach ($v['attrvalue'] as $vv){?>
						<option value="<?php echo $vv?>" <?php if($v['g_type_value']==$vv){?>
                 selected="selected" 
               <?php }?>><?php echo $vv?></option>
				...
				<input type="hidden" name="spec[<?php echo $v["gtid"]?>][attr_type_atid]" value="<?php echo $v['attr_type_atid']?>">
				<td width="280">
					<input type="text" name="spec[<?php echo $v["gtid"]?>][g_other_price]" id="" value="<?php echo $v['g_other_price']?>" placeholder='附加价格' class="form-control"/>	
				</td>
				<td style="text-align: center;">
					<a href="javascript:;" class="btn btn-sm btn-info add-spec">+添加规格</a>
				</td>
			</tr>
		...
		<div lab="uploadFilee" class="uploadFilee">
		    ...
		    <div id="uploadList">
		        <ul>
					...
					<li>
					<img src='<?php echo $goods_all["list_pic"]?>'/>
					<input type='hidden' name='list_pic' value='<?php echo $goods_all["list_pic"]?>'/>
					</li>
		...
		<div lab="uploadFilee" class="uploadFilee">
		    ...
		    <div id="uploadLists">
		        ...
		        <?php foreach ($con_data['m_pics'] as $v){?>
		        	<li>
			        	<img src="<?php echo $v?>"/>
		...
		<table class="table table-hover">
			...
			<tr>
				<!-- 调用百度编辑器 -->
				<script id="editor" type="text/plain" style="width:100%;height:auto;" name="goods_content" rule="required" msg="商品详细必须要填"><?php echo $con_data['goods_content']?></script>
				<script>
		...
		<table class="table table-hover">
			...
			<tr>
				<!-- 调用百度编辑器 -->
				<script id="editor2" type="text/plain" style="width:100%;height:auto;" name="goods_service"><?php echo $con_data['goods_service']?></script>
				<script>
		...
		<input type="hidden" name="type_tid" value="<?php echo $tid?>"/>

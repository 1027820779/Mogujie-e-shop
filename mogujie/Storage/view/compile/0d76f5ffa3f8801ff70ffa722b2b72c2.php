	...
	<body>
		...
		<table class="table table-hover">
			...
			<tr>
				...
				<td><input type="text" name="<?php echo $v['bid']?>" id="" value="<?php echo $v['bsort']?>" class="form-control"></td>
				<td style="text-align:center;"><?php if($v['is_hot']==1){?>
                <i style="color:#1caf9a;" class="icon-flag"></i>
               <?php }?></td>
				<td>
					<a href="<?php echo U('edit',array('bid'=>$v['bid']))?>" class="btn btn-sm btn-warning">编辑</a>
					<a href="javascript:if(confirm('确认删除吗?')) location.href='<?php echo U('del',array('bid'=>$v['bid']))?>';" class="btn btn-sm btn-danger">删除</a>
				</td>
			</tr>
			<?php }?>
		</table>
